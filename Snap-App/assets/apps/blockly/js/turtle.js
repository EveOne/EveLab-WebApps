...
Turtle.setEveBrain = function(evebrain){
...
Turtle.stopButtonClick = function() {
  var runButton = document.getElementById('runButton');
  if(Turtle.evebrain){runButton.style.display = 'inline';}
  ...
  Blockly.mainWorkspace.traceOn(false);
  Turtle.reset();
  if(Turtle.evebrain){Turtle.evebrain.stop();}
};
...
Turtle.animate = function() {
  ...
  if (!tuple) {
    //document.getElementById('spinner').style.visibility = 'hidden';    
    if(Turtle.evebrain){document.getElementById('runButton').style.display = 'inline';}
    document.getElementById('stopButton').style.display = 'none';
  ...
  switch (command) {
    case 'FD':  // Forward
      ...
      if(Turtle.simulation){
        ...
        if(values[0] < 0){
          this.evebrain.back(-values[0], complete);
        }else{
          this.evebrain.forward(values[0], complete);
        }
      }
      break;
    case 'RT':  // Right Turn
      ...
      if(Turtle.simulation){
        ...
        if(values[0] < 0){
          this.evebrain.left(-values[0], complete);
        }else{
          this.evebrain.right(values[0], complete);
        }
      }
      break;
    case 'DP':  // Draw Print
    ...
    case 'PU':  // Pen Up
      ...
      if(Turtle.simulation){
        ...
        this.evebrain.penup(function(res){
          if(res === 'complete'){
            cb();
          }
        });
    ...
    case 'PD':  // Pen Down
      ...
      if(Turtle.simulation){
        ...
        this.evebrain.pendown(function(res){
          if(res === 'complete'){
            cb();
          }
        });
